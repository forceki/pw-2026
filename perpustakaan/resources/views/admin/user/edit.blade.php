        ...
        <div class="col-md-6">
            <div class="card-box">
                ...
                <form method="POST" action="{{ route('admin.user.update', $user) }}">
                    ...
                    <div class="mb-3">
                        <label for="password" class="form-label small fw-semibold">Password <small class="text-muted">(kosongkan jika tidak diubah)</small></label>
                        <input type="password" class="form-control @error('password') is-invalid @enderror"
                               id="password" name="password">
                        @error('password') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <button type="submit" class="btn btn-dark btn-sm">Update</button>
                    <a href="{{ route('admin.user.index') }}" class="btn btn-sm btn-outline-secondary">Batal</a>
                </form>
            </div>
        </div>
    </div>
